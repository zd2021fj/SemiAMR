...
from trainer import SemiAMR

build_model = {
    'SemiAMR': SemiAMR.Trainer,
}
...
def run(config):
    ...
    print("model:{}\tlabel_rate:{}\tprogram seed:{}\tdata seed:{}\tepochs:{}\tsnr:{}\tlearning_rate:{}".format(config.model, config.label_rate, config.program_seed,
           config.data_seed, config.epochs, config.snr, config.lr))
    ...
    # dirs1 = '/data1/ypg/data/RF data/2016.10A/TF/STFTN8'
    # dirs1 = '/data1/ypg/data/RF data/2016.10A/TF/FSST'
    # dirs1 = '/data1/ypg/data/RF data/2016.10A/TF/STFT2'
    # dirs1 = '/data1/ypg/data/RF data/2016.10A/TF/SPWVD'
    # dirs1 = '/data1/ypg/data/RF data/2016.10A/TF/BUD'

    # dirs1 = '/data1/ypg/data/RF data/2016.10A/TF/PWVD'
    # dirs1 = '/data1/ypg/data/RF data/2016.10A/TF/STFTN2'

    dirs1 = dir_list[config.dataset_name]
    ...
    dataloaders['unlabel'] = torch.utils.data.DataLoader(sig_dataset['unlabel'], shuffle=True,
                                                         num_workers=config.workers,
                                                         batch_size=config.usp_batch_size)

    net = arch[config.arch](num_classes=sig_dataset['num_classes'], drop_ratio=config.drop_ratio)
    net = nn.DataParallel(net)
    net = net.to(device)
    optimizer = create_optim(net.parameters(), config)
    scheduler = create_lr_scheduler(optimizer, config)

    net2 = arch[config.arch](num_classes=sig_dataset['num_classes'], drop_ratio=config.drop_ratio)
    net2 = nn.DataParallel(net2)
    net2 = net2.to(device)
    trainer = build_model[config.model](net, net2, optimizer, device, config)
    dir = trainer.loop(config.epochs, sig_dataset, dataloaders['label'], dataloaders['unlabel'], dataloaders['valid'],
                       scheduler=scheduler)


    test.doubel_test(dir, result_modelv1, mods, dataloaders['test'], device, config.version)
